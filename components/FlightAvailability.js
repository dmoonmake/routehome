...
// ─── Main component ────────────────────────────────────────────────────────
export default function FlightAvailability({ route, onClose, defaultDate, defaultAdults }) {
  const date = defaultDate || (() => {
    const d = new Date();
    d.setDate(d.getDate() + 1);
    return d.toISOString().split('T')[0];
  })();
  const adults = defaultAdults || 1;
  ...
  const legParam = stops
    ...
    .join(',');
  const directLegParam = `${stops[0]?.code}-${stops[stops.length - 1]?.code}`;
  // Always check direct flight availability as baseline
  const shouldFetchDirect = !!directLegParam;
  ...
  const fetchAvailability = async () => {
    ...
    try {
      ...
      // Always fetch direct flight for baseline comparison
      const directRes = await fetch(
        `/api/availability?legs=${encodeURIComponent(directLegParam)}&date=${date}&adults=${adults}`
      );
      ...
      if (!res.ok) {
        ...
        setDirectData(directRes.ok ? directJson : null);
      }
    } catch {
  ...
  const routeCheapestTotal = data?.legs?.every((leg) => leg.flights?.length > 0)
    ? data.legs.reduce((sum, leg) => sum + (leg.flights[0]?.price?.total || 0), 0)
    : null;

  const directTopFlight = directData?.legs?.[0]?.flights?.[0] || null;
  const directCheapestTotal = directTopFlight?.price?.total ?? null;
  const directCurrency = directTopFlight?.price?.currency || data?.legs?.[0]?.flights?.[0]?.price?.currency || 'USD';
  ...
  return (
    <div className="mt-3 rounded-2xl border border-blue-200 bg-blue-50/40 overflow-hidden">
      ...
      {/* Search info banner */}
      <div className="px-5 py-3 bg-white border-b border-blue-100 flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-4 text-sm">
          <div className="flex items-center gap-1.5">
            <svg className="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
            </svg>
            <span className="text-slate-600 font-medium">{new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <svg className="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
            </svg>
            <span className="text-slate-600 font-medium">{adults} passenger{adults > 1 ? 's' : ''}</span>
          </div>
        </div>
        <button
      ...
      <div className="p-5">
        ...
        {data && data.legs && (
          <div className="space-y-6">
            {/* Check if entire trip is viable (all legs must have flights) */}
            {(() => {
              const allLegsHaveFlights = data.legs.every((leg) => leg.flights && leg.flights.length > 0);
              const tripIsImpossible = !allLegsHaveFlights;
                ...
                return (
                  <div className="rounded-xl border border-red-200 bg-red-50/80 p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-red-900">No possibility at all</p>
                        <p className="text-xs text-red-700 mt-1">
                          The complete trip {stops[0]?.code} → {stops[stops.length - 1]?.code} cannot be completed on {date}.
                        </p>
                        <p className="text-xs text-red-600 mt-2 font-medium">
                          {data.legs
                            .map((leg, idx) => 
                              !leg.flights?.length > 0 ? `${leg.from}→${leg.to} has no available flights` : null
                            )
                            .filter(Boolean)
                            .join('; ')}
                        </p>
                        <p className="text-xs text-red-600 mt-3">
                          Try another date or check for a direct flight instead.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              // If trip is viable, show comparison and leg details
              return (
                <>
                  {directData && (
                    <div className="rounded-xl border border-indigo-200 bg-indigo-50/60 p-4">
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        <div>
                          <p className="text-sm font-semibold text-indigo-900">Complete Trip vs Direct Flight</p>
                          <p className="text-xs text-indigo-600 mt-0.5">
                            {stops[0]?.code} → {stops[stops.length - 1]?.code} on {date}
                          </p>
                        </div>
                        <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-indigo-100 text-indigo-700">
                          Per person (cheapest options)
                        </span>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                        <div className="rounded-lg bg-white border border-indigo-100 p-3">
                          <p className="text-xs uppercase tracking-wide text-slate-500 font-semibold">Routed Trip</p>
                          <p className="text-lg font-bold text-slate-800 mt-1">
                            {routeCheapestTotal !== null
                              ? formatMoney(routeCheapestTotal, directCurrency)
                              : 'Not possible'}
                          </p>
                          <p className="text-xs text-slate-500 mt-1">
                            {routeCheapestTotal !== null
                              ? `Cheapest option from all ${data.legs.length} leg${data.legs.length > 1 ? 's' : ''}`
                              : 'Not all legs have available flights'}
                          </p>
                        </div>

                        <div className="rounded-lg bg-white border border-indigo-100 p-3">
                          <p className="text-xs uppercase tracking-wide text-slate-500 font-semibold">Direct Flight (Origin to Destination)</p>
                          <p className="text-lg font-bold text-slate-800 mt-1">
                            {directCheapestTotal !== null
                              ? formatMoney(directCheapestTotal, directCurrency)
                              : 'Not available'}
                          </p>
                          <p className="text-xs text-slate-500 mt-1">
                            {directTopFlight
                              ? `${directTopFlight.airlineName || directTopFlight.airline} · ${directTopFlight.duration || '—'} · ${directTopFlight.stops === 0 ? 'Nonstop' : `${directTopFlight.stops} stop(s)`}`
                              : 'No direct flights on this date'}
                          </p>
                        </div>
                      </div>
                    </div>
